...
function MyProjects() {
    ...
    const [projects, setProjects] = useState(projectsData.filter(project => project.filters.includes("FEATURED")));
    const [styles, api] = useSpring(() => ({ opacity: 1 }))
            ...
            <ScaleOnHoverCard className="h-full item-card">
                <div className="flex justify-content-between">
                    ...
                    <div className="h-2rem flex align-items-center">
                        {item.gitHub &&
                            <button style={{'fontSize': '1.2rem'}} onClick={() => window.open(item.gitHub, "_blank")}
                               className="pi pi-github ml-auto cursor-pointer"/>}
                        {item.site && <button style={{'fontSize': '1.2rem'}} onClick={() => window.open(item.site, "_blank")}
                                         className="pi pi-external-link ml-3 cursor-pointer"/>}
                        {item.youtube &&
                            <button style={{'fontSize': '1.2rem'}} onClick={() => window.open(item.youtube, "_blank")}
                ...
                {allVisible && (
                    <div className="grid col-8 my-4 flex justify-content-center" style={{ gridGap: '10px' }}>
                        {projectFilters.map((filter, idx) => (
                            <button key={idx} onClick={() => toggleFilter(filter)}
